	...
	switch(point_number) {
		case 1:		//On Off
			if (prePtbl[GHP_ONOFF_PCM][pno] != g_fExPtbl[GHP_ONOFF_PCM][pno]) {
				...
				printf("Pset On(%d, %d) ", GHP_ONOFF_PCM, GHP_ONOFF_FAN_PCM); 
		...
		case 4:		//Mode
			if (prePtbl[GHP_ONOFF_PCM][pno] != g_fExPtbl[GHP_ONOFF_PCM][pno]) {
				...
				printf("Pset Mode(%d) ", GHP_MODE_PCM); 
				break;
			}
		case 2:		//Set temperature
			if (prePtbl[GHP_ONOFF_PCM][pno] != g_fExPtbl[GHP_ONOFF_PCM][pno]) {
				...
				printf("Pset SetTemp(%d) ", GHP_SET_TEMP_PCM); 
				break;
			}
		...
		case 5:		//Room temperature
			...
			printf("Pset Temp(%d) ", GHP_TEMP_PCM); 
			break;
		...
		case 30:	//Wind Speed
			if (prePtbl[GHP_ONOFF_PCM][pno] != g_fExPtbl[GHP_ONOFF_PCM][pno]) {
				...
				printf("Pset Speed(%d) ", GHP_WINDSPEED_PCM); 
				break;
			}
		...
		case 31:	//Wind Direction
			if (prePtbl[GHP_ONOFF_PCM][pno] != g_fExPtbl[GHP_ONOFF_PCM][pno]) {
				...
				printf("Pset Direction(%d) ", GHP_WINDDIRECTION_PCM);  
				break;
			}
...
/****************************************************************/
{
	...
	st = POLL_START_MSG;
	if(g_dbgShow) printf(" -P-  POLLLING\n");	
	for(;;)
	{
		switch (st)
		{
			...
			case POLL_RECEIVE_EOT:
				if(g_dbgShow) printf(" -P-  POLL_RECEIVE_EOT\n");
				clear_rx_buf();
			...
			case POLL_EOT_MSG:
				if(g_dbgShow) printf(" -P-  POLL_EOT_MSG\n");
				comm_eot_msg();
				if (result == POLL_REQUIRE_STARTUP)
					return POLL_REQUIRE_STARTUP;
				else if (result == POLL_ERROR)
			...
			case POLL_WAIT_15SEC:			
				if(g_dbgShow) printf(" -P-  POLL_WAIT_15SEC\n");
				if (result == POLL_REQUIRE_STARTUP)
					return POLL_REQUIRE_STARTUP;
				else 
					return POLL_NG;
...
/****************************************************************/
{
	...
	if(g_dbgShow) printf(" -S-  SELECTING\n");
	st = SEL_START_MSG;
	for(;;)
	{
		switch (st)
		{
			...
			case SEL_SEND_TEXT:
				if(g_dbgShow) printf(" -S-  SEL_SEND_TEXT\n");
				//SendUart2(p, length);					
				OnWriteOut(p, length);
				for (i = 0; i < length; i++)
					printf("%c ",p[i]);
			...
			case SEL_SEND_EOT:
				if(g_dbgShow) printf(" -S-  SEL_SEND_EOT\n");
				comm_eot_msg();
				return SEL_OK;
		}
	}

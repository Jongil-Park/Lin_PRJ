...
/***********************************************************************/
{
	...
	if( result > 10 || result < 0 ) {
		...
		// '0' is HAKSA MODE.
		result = 0;		
		if (g_fExPtbl[GHP_GROUP_PCM][pno] != 0)
...
/***********************************************************************/
{
	...
	if( result > 2 || result < 0  ) {
		...
		if (g_fExPtbl[GHP_GROUP_MODE_PCM][pno] != 1)
...
/***********************************************************************/
{
	...
	if(group == SCHED_HAKSA_MODE) {
		...
		//g_schedFileData 은 ReadScheduleFile()에서 읽어온다. 
		...
		for (i = 0; i < CLASS_DATA_COUNT; i++) {
			...
			isTime = p->classData[i].startT + p->classData[i].stopT;
			if (isTime == 0)
				continue;

			memset(sched_name, 0x00, sizeof(sched_name));

			for (k = 0; k < 255; k++) {
				if (p->classData[i].classNum == g_UnitData[k].pno) {
					memcpy(sched_name, &g_UnitData[k].name, sizeof(sched_name));

					if ( strncmp(sched_name, (char *)&g_UnitData[index].name, strlen(sched_name)) == 0 ) {
...
/***********************************************************************/
{
	...
	if (g_UnitData[index].chkTouch == 0) {
		...
		if (g_fExPtbl[GHP_ONOFF_PCM][pno] != 0)
...
/***********************************************************************/
{
	...
	if (g_UnitData[index].chkTouch == 0) {
		...
		if (g_fExPtbl[GHP_ONOFF_PCM][pno] != 1)
...
/***********************************************************************/
void Sched_ForceMode_2(int index)
/***********************************************************************/
{
	int pno = 0;
	float value = 0;
	...
	// Timeout이 최초로 발생하면 GHP를 ON 시킨다.
	if (g_UnitData[index].chkTouch != 1) {
...
/***********************************************************************/
{
	...
	for ( i=0; i<MAX_GROUP_COUNT; i++ ) {	
		printf("index(%d) = %d\n", i, pRx->classData[i].index); 	
		printf("classNum(%d) = %d\n", i, pRx->classData[i].classNum); 	
		printf("startT(%d) = %d\n", i, pRx->classData[i].startT); 	
		printf("stopT(%d) = %d\n", i, pRx->classData[i].stopT); 	
	}
	*/
...
/***********************************************************************/
{
	...
	for (i = 0; i < unitCount; i++)
	{
		if (g_UnitData[i].group != SCHED_HAKSA_MODE)
		{
			...
			if (g_fExPtbl[GHP_GROUP_MODE_PCM][g_UnitData[i].pno] != (float)mode)

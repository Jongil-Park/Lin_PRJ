    ...
    for (pno = 0; pno < MAX_DNP_AO_POINT; pno++)
    {
        ...
        if(aio_p->Unsol)
        {
            if(aio_p->Unsol != client_sock)
            {
                ...
                // warnig이 생겨서 제거합니다. 
                //printf("DNPd: Send AO Unsol message, pno %d, PLCaddr %d, value %f, PrevVal %f \n", pno, aio_p->PLCaddr, analog_val, aio_p->PrevVal);
                printf("DNPd: AO Unsol val %f, PrevVal %f \n", aio_p->val, aio_p->PrevVal);
    ...
    //{
        ...
        //{
            ...
            //{
                ...
                analog_val = ai_point;      // to display correctly, analog_val is shifted right at above.
                // warnig이 생겨서 제거합니다. 
                //printf("DNPd: AI Unsol msg, dnpno %d, PLCaddr %d, value %f, PrevVal %f \n", dnpno, aio_p->PLCaddr, analog_val, aio_p->PrevVal);
                printf("DNPd: AI Unsol msg, val %f, PrevVal %f \n", aio_p->val, aio_p->PrevVal);
...
/****************************************************************/
{
    ...
    {   
        ...
        {
            //printf("DEBUG: pno %d \n", pno);
            //p = &g_fExPtbl[dnp_slave_addr][pno];
            //p = &ptbl[pno];
            if(pno < 0)
            {

...
/*******************************************************************/
static void CfgEth_File_Handler(cfg_info *p)
/*******************************************************************/
{
	int i = 0;
	int j = 0;
	...
	if((fp = fopen("/duksan/CONFIG/CfgEth0.sh", "w")) == NULL) {
		printf("[ERROR] File Open with Option 'r'\n");
		return;		
	}
	else {
		fprintf(fp,"ifconfig eth0 down\n");
		//fprintf(fp,"ifconfig eth0 hw ether 00:1C:C0:00:%02d:%02d\n", , );
		for (i = 0; i < cnt; i++) {
			if ( !strcmp(g_temp[i].argv, "ipaddr") ) {
				token = 0;
				//printf("strlen(g_temp[i].argv) = %d\n", strlen(g_temp[i].value));
				for (j = 0; j < strlen(g_temp[i].value); j++) {
					if( g_temp[i].value[j] == '.' ){
						token++;
						if ( token == 3 ) {
							memcpy (ip, &g_temp[i].value[j+1], 3); 
							break;
						}
					}
				}

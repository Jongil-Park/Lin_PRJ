...
int on_Send_To_Kernel(void *p)
{
	...
	if(result != SUCCESS) {
		...
		write ( pNet32->n_clisock, (unsigned char *)pToken, sizeof(NET32_TOKEN_T) ); 
		//send ( pNet32->n_clisock, (unsigned char *)pToken, sizeof(NET32_TOKEN_T), 0 ); 
	...
	write ( pNet32->n_clisock, (unsigned char *)pMsg, sizeof(NET32_MSG_T) ); 
...
int on_Rcv_From_Kernel(void *p)
{
	...
	iRet = recv ( pNet32->n_clisock, pNet32->rxBuf, NET32_BUF_SIZE, 0 );	
...
/*
	pNet32->n_svrsock = socket(PF_INET, SOCK_STREAM, 0);
	
	tv.tv_sec = 0;
	tv.tv_usec = 10000; //0.01sec
	memset ( &pNet32->svr_addr, 0x00, sizeof(pNet32->svr_addr) );

	pNet32->svr_addr.sin_family = AF_INET;
	pNet32->svr_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	pNet32->svr_addr.sin_port = htons(server_port);


	setsockopt(pNet32->n_svrsock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

	if(bind(pNet32->n_svrsock, (struct sockaddr *)&pNet32->svr_addr,
				sizeof(pNet32->svr_addr)) < 0)
	{
		printf("[ERROR] In Bind of Schedule-Server\n");
		close(pNet32->n_svrsock);
		exit(1);
	}

	if(listen(pNet32->n_svrsock, 10) < 0)
	{
	...
	client_addr_size  = sizeof( pNet32->cli_addr);
	pNet32->n_clisock = accept( pNet32->n_svrsock, (struct sockaddr*)&pNet32->cli_addr, &client_addr_size);
*/

	net32_sleep( 2, 0 );


	pNet32->n_clisock = socket(PF_INET, SOCK_STREAM, 0);
	
	tv.tv_sec = 0;
	tv.tv_usec = 10000; //0.01sec
	memset ( &pNet32->cli_addr, 0x00, sizeof(pNet32->cli_addr) );

	pNet32->cli_addr.sin_family = AF_INET;
	pNet32->cli_addr.sin_addr.s_addr = inet_addr("127.0.0.1");
	pNet32->cli_addr.sin_port = htons(9005);

	err = connect( pNet32->n_clisock, (struct sockaddr *) &pNet32->cli_addr, sizeof (pNet32->cli_addr) );
...
void *net32_main(void *arg)
{
	...
	for ( ;; ) {
		...
		on_Send_To_Kernel(pNet32);
		g_nStatusNet32 = 2;
		
		net32_sleep( 0, 1000 );
		g_nStatusNet32 = 3;

		n_ret = on_Rcv_From_Kernel(pNet32);
		g_nStatusNet32 = 4;

		if ( n_ret > 0 ) 
			Handler(pNet32);
		else if ( n_ret == 0 ) {
			fprintf(stderr, "[NET32] : Sock Close...\n");
			fflush(stderr);
		}
		g_nStatusNet32 = 5;

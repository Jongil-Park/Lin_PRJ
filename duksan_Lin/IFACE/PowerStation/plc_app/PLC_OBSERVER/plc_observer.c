...
//
#ifdef DAECHI
#define PLC_DI_ADDR_CNT   3
#define PLC_DO_ADDR_CNT   5   
#define PLC_AI_ADDR_CNT   1  
#define PLC_AO_ADDR_CNT   1 
  
static int PLC_DI_ADDR[PLC_DI_ADDR_CNT]  = {  128,  192,  256 };
static int PLC_DI_COUNT[PLC_DI_ADDR_CNT] = {   64,   64,   64 };
static int PLC_DI_POINT[PLC_DI_ADDR_CNT] = {    0,   64,  128 };

static int PLC_DO_ADDR[PLC_DO_ADDR_CNT]  = { 2880, 2944, 2480, 2544, 1328 };
static int PLC_DO_COUNT[PLC_DO_ADDR_CNT] = {   64,   64,   64,   64,   16 };
static int PLC_DO_POINT[PLC_DO_ADDR_CNT] = {    0,   64,  128,  192,  256 };

static int PLC_AI_ADDR[PLC_AI_ADDR_CNT]  = {  300 };
static int PLC_AI_COUNT[PLC_AI_ADDR_CNT] = {   40 };
static int PLC_AI_POINT[PLC_AI_ADDR_CNT] = {    0 };

static int PLC_AO_ADDR[PLC_AO_ADDR_CNT]  = {  500 };
static int PLC_AO_COUNT[PLC_AO_ADDR_CNT] = {   74 };
static int PLC_AO_POINT[PLC_AO_ADDR_CNT] = {    0 };
#endif
//
// 8. 대치변전소 - start
...
void byun_jun_so_bongchun()
{
    ...
    for (i = 0; i < 32; i++)
        PLC_DIO_define(DNP_DO_SEL, i+224, i+768, i+766, i+767, 1);    
...
void byun_jun_so_daechi()
{
    ...
    for(i = 0; i < 64; i++)
        PLC_DIO_define(DNP_DO_SEL, i,     i+2880, i+480, i+880, 0);
    for(i = 0; i < 64; i++)
        PLC_DIO_define(DNP_DO_SEL, i+64,  i+2944, i+544, i+944, 0);
    for(i = 0; i < 64; i++)
        PLC_DIO_define(DNP_DO_SEL, i+128, i+2480, i+1680, i+2080, 1);
    for(i = 0; i < 64; i++)
        PLC_DIO_define(DNP_DO_SEL, i+192, i+2544, i+1744, i+2144, 1);
    for(i = 0; i < 16; i++)
        PLC_DIO_define(DNP_DO_SEL, i+256, i+1328, i+528, i+928, 0);
    
    for(i = 0; i < 40; i++)
        PLC_AIO_define(DNP_AI_SEL, i, i+300, 1, 0, 0, 16000);
    
    PLC_AIO_define(DNP_AI_SEL, 24, 324, 1, 0, -300, 15700);  /* Outer temperature */
...
#endif		
	{
		for(i=0;i<(lCount/8);i++) // byte 갯수만큼
		{
			for(j=0;j<8;j++) // 1바이트를 => 비트로 분리
			{
				...
				{
					DNP_p->val = 1;
					//printf(" [%d] [%d] On \n", lPoint, DNP_do_point[lPoint].PLCaddr);
				}
				else {
					DNP_p->val = 0;
					//printf(" [%d] [%d] Off \n", lPoint, DNP_do_point[lPoint].PLCaddr);
...
/****************************************************************/
{
    ...
    for (i = 0; i < MAX_DNP_AO_POINT; i++)
    {
        ...
        if(aio_p->WrFlag)
        {
            WriteAOPoint(plc_socket, i, aio_p->PrevVal);
            aio_p->WrFlag = 0;
...
/****************************************************************/
{
	...
	//
	//
	printf("Start PLC While Loop - 090803 Type A\n");
	//
